...
export default function ClientTable() {
	...
	return (
		<div className={`relative flex flex-col h-full w-full`}>
			<div
				className="fixed flex-col top-0 bg-secondary/60 backdrop-blur-sm text-base-content flex justify-between items-center  w-full text-sm shadow-lg">
				<SearchField />
								...
								{viewMode === 'grouped' ? (
									Object.entries(clientsToShow).map(([status, clients], idx) => (
										<React.Fragment key={status}>
											...
											{clients.map((person, i) => (
												<ClientTableItem key={`${idx}-${i}`} person={person} i={i}
									...
									Array.isArray(clientsToShow) && clientsToShow.length > 0 ? (
										clientsToShow.map((person, i) => (
											<ClientTableItem key={i} person={person} i={i}
											                 statusCollapse={statusCollapse} />
										))
									) : (
										<tr>
