	...
	if (type === 'textarea') {
		return (
			<div className="relative ">
				...
				>{label}</label>
				<textarea name={name} id={name} placeholder={label} value={value} onChange={handleChange}
		...
		return (
			<div className="relative">
				<label
					...
					className="absolute z-10 -top-2 left-2 inline-block rounded-lg px-1 text-xs font-medium text-base-content bg-base-300"
				>{label}</label>
				<select autoComplete={'true'} name={name} id={name} value={value} onChange={handleChange}
