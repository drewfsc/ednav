  ...
  try {
    ...
    if (user) {
      ...
      dataRes = await clientsCollection.updateOne(
        { _id: clientId ? new ObjectId(clientId) : undefined },
        { $set: { trackable: body.trackable } },
      );
    }
    if (
      ...
      dataRes = await clientsCollection.updateOne(query, {
        $set: {
          orientation: {
    ...
    return NextResponse.json(
      { message: "Action added successfully", dataRes, user },
      { status: 201 },
    );
  } catch (error) {
