...
export async function GET(req: Request) {
  ...
  const graduatedClientsPipeline = [
    {
      $match: {
        what: { $regex: "^graduated$", $options: "i" },
  ...
  const clientStatusData = await clientsCollection
    .aggregate(clientStatusPipeline)
    .toArray();
  const clientsReferredData = await clientsCollection
    .aggregate(clientsReferredPipeline)
    .toArray();
  const graduatedClientsData = await actionsCollection
    .aggregate(graduatedClientsPipeline)
    .toArray();
  const enrolledClientsData = await actionsCollection
    .aggregate(enrolledClientsPipeline)
    .toArray();
  const clientsByRegionData = await clientsCollection
    .aggregate(clientsByRegionPipeline)
    .toArray();
  ...
  const totalClients = clientStatusData.reduce(
    (sum, { count }) => sum + count,
    0,
  );
  const clientStatusPercentages = clientStatusData.map(({ _id, count }) => ({
    status: _id,
    percentage:
      totalClients > 0 ? ((count / totalClients) * 100).toFixed(2) + "%" : "0%",
  }));
  ...
  const graduatedClientsPerMonth = graduatedClientsData.map(
    ({ _id, count }) => ({
      month: _id,
      count,
    }),
  ...
  const totalClientCount =
    (await clientsCollection.countDocuments({
